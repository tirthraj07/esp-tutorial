...
void build_dynamic_mqtt_topics(){
    ...
    sprintf(topic_commands, "nodes/%s/commands", NODE_ID);
    sprintf(topic_ping, "nodes/%s/ping", NODE_ID);

    printf("Assigned Telemetry Topic: %s\n", topic_telemetry);
    printf("Assigned Commands Topic: %s\n", topic_commands);
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    switch ((esp_mqtt_event_id_t)event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            // 1. Subscribe to our dynamic "commands" topic
            esp_mqtt_client_subscribe(client, topic_commands, 0);
            // 1. Subscribe to our dynamic "ping" topic
            esp_mqtt_client_subscribe(client, topic_ping, 0);
            
            // 2. Spawn the Background Telemetry Task
...
void handle_mqtt_data_event(esp_mqtt_event_handle_t event){
    if (is_event_on_topic(event, topic_commands)) {
        printf("Routing to: COMMAND_HANDLER\n");
        handle_data_on_command_topic(event);
    } else if (is_event_on_topic(event, topic_ping)){ 
